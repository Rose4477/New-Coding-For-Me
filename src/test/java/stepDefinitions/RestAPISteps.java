...
public class RestAPISteps {
	...
	private final String AccessToken = "?access-token=39fc80cb28f3f8501fe24366a66bf297981beb93c142cd3a4270546d878b3fd2";
	private final String BASE_URL = "https://gorest.co.in/public/v2/users";
	private Response response;
	private String[] newUser;
	private int newUserID;
	private String newTestID;
	private String newName = "New User";
	...
	public void new_user_data_from_the_csv_file_and_csv_row(String fileName, int rowNumber)
		...
		CSVReader csvReader = new CSVReaderBuilder(new FileReader(
				"C:\\Users\\rose.thomas\\eclipse-workspace\\RestAssuredFramework\\src\\test\\resources\\Input\\UserData.csv"))
				.withSkipLines(1).withCSVParser(csvParser).build();
	...
	public void add_a_new_user_with_that_data() {
		...
		response = request.body(jsonStringFromRequest).post(BASE_URL + AccessToken);
	...
	public void a_change_is_made_to_this_new_user() {
		...
		RequestSpecification request = RestAssured.given();
		request.header("Content-Type", "application/json").contentType(ContentType.JSON).accept(ContentType.JSON);
		response = request.body(requestJSonObj.toString()).put(BASE_URL + "/" + newUserID + AccessToken);
